...
function pickPriceBySymbol(attrs: any, symbol?: string | null): number | null {
  ...
  if (symbol) {
    ...
    if (s && baseSym === s && isFinite(baseUsd)) return baseUsd
    if (s && quoteSym === s && isFinite(quoteUsd)) return quoteUsd
  }
...
export async function getGeckoPoolQuote(network: string, poolAddress: string, tokenSymbol?: string | null): Promise<GeckoQuote | null> {
  ...
  try {
    ...
    const change = Number(attrs?.price_change_percentage?.h24 ?? attrs?.price_change?.h24 ?? NaN)
    return {
      network: net,
      pool: addr,
      priceUsd: picked as number,
