...
const DEFAULT_AVATAR = '/avatars/default-avatar.png'
  ...
  async function registerUser() {
    ...
    if (name.length < 2) { alert('Enter a valid username'); return }
    if (wallet.length < 6) { alert('Enter a valid wallet address'); return }
    const newUser = { id: wallet, username: name, wallet, createdAt: Date.now(), lastLogin: Date.now(), avatar: DEFAULT_AVATAR }
    resetPersistentState()
    try { localStorage.setItem('flipflop-user', JSON.stringify(newUser)) } catch {}
    setUser(newUser)
    setRegisterOpen(false)
    
    // Register user on server to get initial points and pack
    try {
      const r = await fetch(`/api/users/me?userId=${encodeURIComponent(wallet)}`)
      const j = await r.json()
      if (j?.ok && j?.user) {
        // Points are granted on server side, fetch them
        const bankPoints = j.user.bankPoints || 0
        setPoints(bankPoints)
        try {
          localStorage.setItem('flipflop-points', String(bankPoints))
        } catch {}
        // Enable starter pack claim
      ...
      {registerOpen && (
        <div className="modal-backdrop" onClick={()=>setRegisterOpen(false)}>
          <div className="modal" onClick={(e)=>e.stopPropagation()} style={{maxWidth: 520, width: '92%'}}>
            <div className="modal-header">
              <h3 style={{color:'white', fontSize:18, fontWeight:800, margin:0}}>Create Account</h3>
              <button onClick={()=>setRegisterOpen(false)} style={{background:'none', border:'none', color:'white', fontSize:20, cursor:'pointer'}}>×</button>
            </div>
            <div className="modal-body">
              <div style={{display:'grid', gap:12}}>
                <div>
                  <label style={{display:'block', fontSize:12, opacity:.8, marginBottom:6}}>Username</label>
                  <input value={regName} onChange={e=>setRegName(e.target.value)} placeholder="Your nickname" style={{width:'100%'}} />
                </div>
                <div>
                  <label style={{display:'block', fontSize:12, opacity:.8, marginBottom:6}}>Wallet Address</label>
                  <input value={regWallet} onChange={e=>setRegWallet(e.target.value)} placeholder="0x..." style={{width:'100%'}} />
                </div>
                <div style={{fontSize:12, color:'var(--muted-inv)'}}>One wallet can register once during beta. You receive 50,000 points and 1 starter pack.</div>
              </div>
            </div>
